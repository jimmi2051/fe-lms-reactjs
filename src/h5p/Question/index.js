...
class Question extends Component {
  ...
  render() {
      ...
      return (
        <div className="content-question">
          <div className="content-question__warp-result">
            ...
            <p className="text-center font-weight-bold">{countCorrect} of {questions.length} | {percentResult} %</p>
            <p className="text-center text-success font-weight-bold">{comment}</p>
          </div>
          <h5 className="">Check your answer</h5>
          {questions && questions.map((item, index) => {
            return (
              <div className="content-question__question">
                <p className="content-question__t-question">Question: {index + 1}: {item.question}</p>
                <div className="row">
                  {item.answers &&
                    item.answers.map((answer, indexAnswer) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`warp-answer ${answer.result ? "warp-answer-active" : ""}
    ...
    return (
      <div className="content-question">
        <h5 className="content-question__title">Question: {currentQuestion + 1} of {questions.length}</h5>
        {currentQuestion < questions.length && (
          <div className="content-question__question">
            <p className="content-question__t-question">{questions[currentQuestion].question}</p>
            {missAns && (
              <p className="text-danger">You must choose one answer.</p>
            )}
            <div className="row">
              {questions[currentQuestion].answers &&
                questions[currentQuestion].answers.map((answer, indexAnswer) => {
                  const findIndex = _.findIndex(
                    listAnswer,
                    itemAnswer =>
                      _.isEqual(itemAnswer.answer, answer) &&
                      _.isEqual(itemAnswer.question, questions[currentQuestion])
                  );
                  let check;
                  if (findIndex > -1) {
                    check =
                      listAnswer[findIndex].answer === answer &&
                        listAnswer[findIndex].question === questions[currentQuestion]
                        ? true
                        : false;
                  } else {
                    check = false;
                  ...
                  return (
                    <div
                      className={`content-question__t-answer col-xl-6`}
                      key={indexAnswer}
                      onClick={() => this.handleChooseAnswer(questions[currentQuestion], answer)}
                    >
                      <div className={`warp-answer ${check ? "warp-answer-active" : ""}`}>{answer.anw}</div>
                    </div>
                  )
                })}
            </div>
